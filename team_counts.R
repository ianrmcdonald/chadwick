...
batting_id_teams_year <- inner_join(batting_raw, team_1, by = "teamID",
                               relationship = "many-to-many") %>% 
  select(playerID, franchID, yearID) %>% 
  distinct()

pitching_id_teams_year <- inner_join(pitching_raw, team_1, by = "teamID",
                                relationship = "many-to-many") %>% 
  select(playerID, franchID, yearID) %>% 
  distinct()
...
two_team_list <- find_all_two_teams("CLE", "PHI")
...
threshhold_team <- function(team, threshhold_i, stat, type="batting") {
  ...
  threshhold_list <- table_data %>%
        inner_join(., team_1, by = "teamID", relationship = "many-to-many") %>% 
...
threshhold_list <- threshhold_team("OAK", 40, "HR")
...
#filter those who played for just one team

#find_award_winners requires winner be with team in a particular year
